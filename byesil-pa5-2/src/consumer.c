...
static int do_procs(int fd) {
    ...
    char *buf = malloc(size + 1); // Allocate buffer to hold one data element
    int count;

    // Create 'g_concurrency' number of child processes
    for(i = 0; i < g_concurrency; i++) {
        pid = fork();

        if(pid == 0) { // Child process
            // Read one data element from the FIFO
            printf("read: ");
            if((count = read(fd, buf, size)) < 0)
                perror("read"); // Print error message if read fails
            if(count) { // Print the data element if it was read successfully
                buf[size] = '\0'; // Null-terminate the data element
                printf("%s\n", buf);
            }
